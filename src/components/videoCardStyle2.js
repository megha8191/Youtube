...
const VideoCardStyle2 = ({item}) => {
  ...
  return (
    ...
    <div className='relative w-full'>
        {/* <p className="text-xs bg-black bg-opacity-80 text-white absolute bottom-1 py-1 px-2 rounded right-1 ">{convertDurationToTime(contentDetails.duration)}</p> */}
    ...
    <div className='py-1'>
        ...
        {/* <p className="text-gray-500 font-xs pt-1 -mt-1 gap-1 inline-flex flex-wrap line-clamp-1"><span>{formatViewCount(statistics.viewCount)}  views</span> &bull; <span>8 months ago</span></p> */}
        <div className="w-100 flex sm:my-3 my-1 gap-2 items-center">
            ...
            <div className=" basis-5/6 col-span-11 ">
                <p className="text-gray-500 font-xs">{channelTitle}</p>
             </div>
        </div>
        <p className='font-xs text-gray-600 line-clamp-1'>{description}</p>
    </div>
